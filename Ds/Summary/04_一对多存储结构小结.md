...
* **检查**红黑特性满足条件，**决策**执行自平衡操作。
  ...
  * 父亲为红色 ==> 一定有**黑色的祖父**结点。
    * 叔叔为红色。
      ...
      ![红黑树_插入_父结点红_叔叔结点红](./images/红黑树_插入_父结点红_叔叔结点红.svg)
    ...
    * 叔叔为黑色（包括Null）
      ...
      * 祖父、父亲、孩子**不同向**
        ...
        2. **按同向逻辑**变色和旋转。
           ...
           ![红黑树_插入_父亲为红_叔叔为黑](./images/红黑树_插入_父亲为红_叔叔为黑.svg)
...
* 删除自平衡操作。
  ...
  1. 替代结点X 为红色 ==> 变黑即可
     ...
     ![红黑树_删除自平衡_替代结点为红色](./images/红黑树_删除自平衡_替代结点为红色.svg)
  ...
  2. 替代结点为黑色 
     ...
     1. 兄弟S为红色 ==> **父亲P，左侄子SL，右侄子SR一定为黑色**
        ...
        * 兄弟S，父亲P互换颜色；
        * 对P旋转，**当前结点**变为**X的兄弟结点**。
        * 此时所有路径**黑结点数未受影响**。经过路径①②的黑结点，仍然比其他路径少1个。
